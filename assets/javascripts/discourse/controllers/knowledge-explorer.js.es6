...
export default Ember.Controller.extend({
  ...
  canFilterSolved() {
    return (
      ...
      this.siteSettings.chord_directory_add_solved_filter
    );
  },
  ...
  actions: {
    setSelectedTopic(topicId) {
      ...
      ChordDirectory.getTopic(topicId).then(result => {
        this.setProperties({
    ...
    loadMore() {
      if (this.canLoadMore && !this.isLoadingMore) {
        ...
        ChordDirectory.loadMore(this.loadMoreUrl).then(result => {
    ...
    refreshModel() {
      ...
      ChordDirectory.list(params).then(result => {
        result = mergeCategories(result);
        this.setProperties({
          model: result,
          isLoading: false
